...
router.get("/showAll/:pageIndex", async function (req, res) {
  const { pageIndex } = req.params;
  const pageSize = 4;
  let factor = 0;
  let user = null;
  let recordsCount = await Articles.count();
  if(recordsCount > 0) {
    let numberOfPages = utility.getNumberOfPages(recordsCount, pageSize);
    let articles = [];
    if(pageIndex === '1') {
      articles = await Articles.find({}).sort({ _id: -1 }).limit(pageSize);
    } else {
      let pageIndexNum = +pageIndex;
      let startIndex = (pageIndexNum - 1) * pageSize;
      articles = await Articles.find().skip(startIndex).sort({ _id: -1 }).limit(pageSize);
    }
...
router.post("/findBySearhTerm/:pageIndex", async function (req, res) {
    const { pageIndex } = req.params;
    const pageSize = 4;
    let factor = 0;
    let user = null;
    const searchTerm = req.body.searchTerm;
    let recordsCount = await Articles.find({ $or: [
    ...
    let articles = [];
    if(pageIndex === '1') {
      articles = await Articles.find({ $or: [
        { title: new RegExp(searchTerm, "i") },
        { owner: new RegExp(searchTerm, "i") },
        { content: new RegExp(searchTerm, "i") }
      ...
      articles = await Articles.find({ $or: [
        ...
        { owner: new RegExp(searchTerm, "i") },
        { content: new RegExp(searchTerm, "i") }
      ]}).skip(startIndex).sort({ _id: -1 }).limit(pageSize);
    }
    articles = utility.configureDisplayContent(articles);
    if (req.session.LoggedInUser) {
      factor = 1;
...
router.get("/favourites/:id/:pageIndex", async function (req, res) {
    const { pageIndex } = req.params;
    const pageSize = 4;
    const id = req.params.id;
    const user = await Users.findById(id);

    let recordsCount = await Articles.find({ likes: user.Username }).count();
    let numberOfPages = utility.getNumberOfPages(recordsCount, pageSize);
    let favourites = [];
    if(pageIndex === '1') {
      favourites = await Articles.find({ likes: user.Username }).sort({ _id: -1 }).limit(pageSize);
    } else {
      let pageIndexNum = +pageIndex;
      let startIndex = (pageIndexNum - 1) * pageSize;
      favourites = await Articles.find({ likes: user.Username }).skip(startIndex).sort({ _id: -1 }).limit(pageSize);
    }
    ...
    res.render("articles/favourites.ejs", { 
      numberOfPages,
      recordsCount,
      pageIndex,
      pageSize,
...
router.get("/addToFavourites/:id/:username", async function (req, res) {
    ...
    if (temp == 1) {
      ...
      res.redirect("/articles/showAll/1");
    } else {
...
router.get("/postArticle", function (req, res) {
  ...
  if (req.session.LoggedInUser) {
    let topic = "india usa pakistan china";
